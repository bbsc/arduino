...
int blank[] = { OF, OFF, OFF, OFF, OFF, OFF, OFF, OFF };
...
//   digits[0] = zero
int *digits[] = { zero, one, two, tree, four, five, six, seven, eight, nine, blank};
...
void loop() {
  set_place_values()
  light_3_displays(place_values[0], place_values[], place_values[2]);
}
...
int light_3_displays(int ones_digit, int tens_digit, int hundreds_digit) {
  in display_delay_time = 500;
  int segment_delay_time = 500;
  
  select_displays(ONES_DISPLAY);
